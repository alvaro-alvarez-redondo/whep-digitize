...
| `performance` | `ingest_workbook_batch_size=32`, `ingest_parallel_workers="auto"`, `ingest_parallel_workers_auto_max=8`, `import_future_scheduling=4`, normalize thresholds |
...
| `postpro` | rule columns, wildcard `#ANY#`, `rule_match_normalization`, `target_update_strategies` (default `token_substitute`; `notes`→`concatenate`), `multi_pass` (max 10, `cycle_policy="warn"`), `runtime_cache`/`schema_validation_cache` (both off), audit file names |
| `output_config` | `lists_to_export`, `output_layers=("raw","clean","normalize","harmonize")`, `processed_suffix=".tsv"` |
...
|--------|---------|---------|----------|
| `drop_na_values` | `WHEP_DROP_NA_VALUES` | `True` | drop null-`value` rows during ingest |
| `progress_enabled` | `WHEP_PROGRESS_ENABLED` | `True` | show the `rich` progress display |
| `checkpointing_enabled` | `WHEP_CHECKPOINTING_ENABLED` | `False` | cache the ingest stage: restore a prior `InputResult` instead of re-running, save one on completion |
| `ingest_parallel_workers` | `WHEP_INGEST_PARALLEL_WORKERS` | `"auto"` | import worker count (`"auto"`→`min(8, cpu-1)`; `1`=sequential) |
...
`checkpointing_enabled` is wired in `ingest.runner.run_ingest_pipeline` only; postpro and export
