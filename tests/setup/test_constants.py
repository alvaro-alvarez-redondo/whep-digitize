...
def test_pinned_scalar_values() -> None:
    ...
    assert constants.performance.ingest_workbook_batch_size == 32
    assert constants.performance.ingest_parallel_workers == "auto"
    assert constants.defaults.notes_value is None
    # Only the ingest stage checkpoints, under this name.
    assert constants.checkpoints.ingest_stage_name == "ingest_pipeline"
...
def test_alias_and_output_config() -> None:
    constants = get_pipeline_constants()
    assert constants.header_normalization.canonical_aliases["country"] == "polity"
    assert constants.output_config.output_layers == ("raw", "clean", "normalize", "harmonize")
    assert constants.output_config.processed_suffix == ".tsv"
