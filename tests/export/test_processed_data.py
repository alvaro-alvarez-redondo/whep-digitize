...
def test_build_path_naming(config: Config) -> None:
    ...
    assert path.parent == config.paths.data.output.processed
...
def test_export_writes_every_layer_by_default(config: Config) -> None:
    config.paths.data.output.processed.mkdir(parents=True, exist_ok=True)
...
def test_export_honors_config_output_layers(config: Config) -> None:
    config.paths.data.output.processed.mkdir(parents=True, exist_ok=True)
    output_config = dataclasses.replace(
        config.output_config, output_layers=("raw", "clean", "harmonize")
    )
    wide_config = dataclasses.replace(config, output_config=output_config)
...
def test_export_no_matching_layer_raises(config: Config) -> None:
    ...
    output_config = dataclasses.replace(config.output_config, output_layers=("harmonize",))
    narrow = dataclasses.replace(config, output_config=output_config)
